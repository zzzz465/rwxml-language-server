...
function ObjToMap(object: any): Map<string, CompletionItem> {
  ...
  for (const value of Object.values<CompletionItem>(object)) result.set(value.label, value)
...
export class TypeInfo implements TypeInfo {
  ...
  toString(): string {
    const text = [`typeId: ${this.typeIdentifier}`]
...
export function objToTypeInfos(raw: any): TypeInfo[] {
  const types = Object.values(raw).map((data) => new TypeInfo(data))
  return types
}
...
export class TypeInfoMap extends Map<string, TypeInfo> {
  ...
  constructor(typeInfos: TypeInfo[]) {
    ...
    for (const typeInfo of typeInfos) {
      ...
      if (typeInfo.specialType) {
        ...
        if (defType) {
          if (!this.typeMap.has(defType)) this.typeMap.set(defType, typeInfo)
          else console.log(`duplicate defType ${defType}`)
        }

        if (typeInfo.specialType.compClass) {
          let name: string
          if (typeInfo.typeIdentifier.match(/^Verse|^RimWorld/)) name = typeInfo.typeIdentifier.split('.').reverse()[0]
          else name = typeInfo.typeIdentifier
          ...
          if (!map.has(name)) map.set(name, typeInfo)
          else console.log(`duplicate comp className ${name}`)
        }
      }
    }
  }
  ...
  *getComps(baseType?: TypeIdentifier): Generator<[string, TypeInfo], void, unknown> {
    if (baseType) {
      ...
      for (const map of this.compMap.values()) for (const entry of map) yield entry
    }
  }
}
  ...
  private _convertInternal(node: typeNode): void {
    ...
    while (queue.length > 0) {
      ...
      if (typeInfo.specialType?.enumerable && typeInfo.specialType.enumerable.isSpecial !== true) {
        ...
        for (const childNode of node.children) queue.push(Object.assign(childNode, { typeInfo: childType }))
      }
      ...
      if (ClassValue) {
        ...
        if (specialTypes?.compClass) {
          ...
          if (values.length > 1) {
            ...
            // Find in Verse / RimWorld
            newType =
              this.typeInfoMap.getByTypeIdentifier(`RimWorld.${ClassValue}`) ||
        ...
        if (!injected)
          // do we have to delete this? really?
