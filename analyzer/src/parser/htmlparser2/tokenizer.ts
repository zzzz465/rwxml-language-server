...
const stateBeforeScript1 = ifElseState('R', State.BeforeScript2, State.InTagName)
const stateBeforeScript2 = ifElseState('I', State.BeforeScript3, State.InTagName)
const stateBeforeScript3 = ifElseState('P', State.BeforeScript4, State.InTagName)
const stateBeforeScript4 = ifElseState('T', State.BeforeScript5, State.InTagName)
...
const stateBeforeSpecialT = ifElseState('I', State.BeforeTitle1, State.InTagName)
...
export class Tokenizer {
  ...
  private stateBeforeClosingTagName(c: number) {
    if (whitespace(c)) {
      ...
      } else if (this.special === Special.Title && (c === CharCodes.LowerT || c === CharCodes.UpperT)) {
