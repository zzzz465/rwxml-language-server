...
export class CustomTextDocuments {
  ...
  get ReferenceDocumentsAdded(): iEvent<DocumentAddedEvent> {
    return this._ReferenceDocumentsAdded
  }

  private _DocumentAdded = new Event<DocumentAddedEvent>()
  get DocumentAdded(): iEvent<DocumentAddedEvent> {
    return this._DocumentAdded
  }

  private _DocumentChanged = new Event<DocumentChangedEvent>()
  get DocumentChanged(): iEvent<DocumentChangedEvent> {
    return this._DocumentChanged
  }

  private _DocumentDeleted = new Event<DocumentDeletedEvent>()
  get DocumentDeleted(): iEvent<DocumentDeletedEvent> {
    return this._DocumentDeleted
  }

  GetDocument(documentUri: DocumentUri): TextDocument | undefined {
    return (
      this.editorDocuments.get(documentUri) ||
      this.fileDocuments.get(documentUri)
    )
  }
  ...
  listen(connection: IConnection): void {
    connection.onNotification(
      DefFileAddedNotificationType,
      this.OnDocumentAdded.bind(this)
    )
    connection.onNotification(
      DefFileChangedNotificationType,
    ...
    connection.onNotification(
      DefFileRemovedNotificationType,
      this.OnDocumentRemoved.bind(this)
    )
    // 레퍼런스용 코드
    connection.onNotification(
      ReferencedDefFileAddedNotificationType,
    ...
    for (const [uri, text] of Object.entries(params.files)) {
      ...
      if (doc) {
        TextDocument.update(
          ...
          [
            {
              // just replace entire text
              range: {
                start: { line: 0, character: 0 },
                end: { character: 0, line: doc.lineCount },
              },
              text,
            },
          ],
        ...
        if (!this.editorDocuments.has(uri))
          // opened documents 는 editor 쪽에서 invoke함
          this._DocumentChanged.Invoke({ textDocument: doc })
      } else {
        throw new Error(
          `document ${uri} was not registered but raised during OnDocumentChanged Event`
        )
      }
    }
  }
  ...
  private OnEditorDocumentChanged({
    contentChanges,
    textDocument,
    ...
    if (doc) {
      ...
      throw new Error(
        `document ${textDocument.uri} was not registered but raised during OnEditorDocumentChanged`
      )
    }
  }

  private OnEditorDocumentClosed({
    textDocument,
  }: DidCloseTextDocumentParams): void {
