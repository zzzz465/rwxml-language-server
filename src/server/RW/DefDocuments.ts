...
export class DefDocuments {
  ...
  DocumentAdd(document: TextDocument): DefDocument {
    const defDocument = this.parseText(
      document.getText(),
      document.uri,
      this.typeInjector
    )
  ...
  private parseText(
    ...
    const defDoc = Object.assign(parse(content, uri), {
      defs: [],
    }) as DefDocument
    if (defDoc.root?.tag?.content === "Defs") {
      // if the root node is <Defs>
      for (const node of defDoc.root.children) {
        injector.Inject(node)
        if (isDef(node)) defDoc.defs.push(node)
      }
    }
