...
export class Directory {
  ...
  Delete(obj: File | Directory): void {
    ...
    else _.remove(this._directories, (d) => d.Uri.path === obj.Uri.path)
  }

  Get(pattern: RegExp): File | Directory | undefined {
    return (
      this._directories.find((d) => !!pattern.exec(d.name)) ||
      this._files.find((d) => !!pattern.exec(d.name))
    )
  }
...
export class RootDirectory {
  ...
  Add(file: File): void {
    ...
    if (root) {
      ...
      const paths = Path.dirname(Path.relative(root.path, file.Uri.path))
        .split(Path.sep)
        .map((d) => toFirstLetterUppercase(d)) // paths without last basename
  ...
  private GetRelatedRoot(uri: URI): URI | null {
    for (const parent of this._roots.values()) {
      if (isSubPath(parent.path, uri.path)) return parent
    }
