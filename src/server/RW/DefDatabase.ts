...
export class DefDatabase implements iDefDatabase {
  ...
  getByURI(URILike: DocumentUri): def[] {
    // only returns valid def
    return this._defs.get(URILike) || []
  }

  getByName(defType: defType, name: string): def | null {
    // defName or Name property
  ...
  update(documentUri: DocumentUri, newDefs: def[]): Set<DirtyNode> {
    ...
    for (const def of newDefs) {
      if (!def.tag || !def.closed) continue
      ...
      if (def.attributes?.ParentName) this.registerInheritWanter(def)

      const weakRefWanters = BFS(def)
        .filter<typeNode>(isTypeNode)
    ...
    // merging two sets
    for (const val of dirty2) dirty1.add(val)
  ...
  private resolveWeakRefWanters(): Set<DirtyNode> {
    ...
    for (const refNode of this._weakDefRefWanters.values()) {
      ...
      if (refNode.typeInfo.specialType?.defType) {
        ...
        if (refNode.text && refNode.text.content) {
  ...
  private CastOrConvertToWeakRefNode(node: typeNode): WeakRefNode {
    ...
    if (!isWeakRefNode(node))
      refNode = Object.assign(node, {
        weakReference: { in: new Set(), out: new Set() },
      } as iWeakRef)
    else refNode = node
    ...
    if (defs) {
      for (const def of defs) {
        ...
        if (map) {
          ...
          if (defName) {
            map.delete(defName)
            if (map.size == 0) this._defDatabase.delete(def.tag.content)
          }
        }

        if (isReferencedDef(def)) {
          this.disconnectReferences(def).map((node) => dirtyNodes.add(node))
          // remove references
          this._inheritWanters.delete(def)
        ...
        function GCIfZero(node: WeakRefNode): void {
          if (
            node.weakReference.in.size == 0 &&
            node.weakReference.out.size == 0
          )
  ...
  private disconnectReferences(def: InheritDef): DirtyNode[] {
    ...
    if (def.base)
      // remove cross-reference with parent
      assert(
        def.base.derived.delete(def),
        `tried to remove parent reference ${def.tag} which is not valid`
      )

    for (const derived of def.derived.values()) {
      // remove cross-reference with child
