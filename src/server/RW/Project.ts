...
export class Project implements Disposable {
  ...
  public get Change(): iEvent<ProjectEvent> {
    return this._Change
  }
  ...
  private registerEventHandlers() {
    this.TextDocuments.DocumentAdded.subscribe(
      this,
      this.onDefFileAdded.bind(this)
    )
    this.TextDocuments.DocumentChanged.subscribe(
      this,
      this.onDefFileChanged.bind(this)
    )
    this.TextDocuments.DocumentDeleted.subscribe(
      this,
      this.onDefFileDeleted.bind(this)
    )
    this._TextureAddEvent.subscribe(this, (files) =>
      files.map((file) => this.onFileAdded(file, FileKind.Texture))
    )
    this._TextureDeleteEvent.subscribe(this, (files) =>
      files.map((file) => this.onFileDeleted(file, FileKind.Texture))
    )
  }
  ...
  private onDefFileAdded(event: DocumentAddedEvent): void {
    if (event.isReferenced) this.referenced.add(event.textDocument.uri)

    const defDocument = this.DefDocuments.DocumentAdd(event.textDocument)
    const dirtyNodes = this.DefDB.update(
