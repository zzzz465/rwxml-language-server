...
import {
  ...
  TextureRemovedNotificationType,
...
function ValidateDirtyNodes(
  dirtyNodes: DirtyNode[],
  project: Project
): PublishDiagnosticsParams[] {
  const documents = AsEnumerable(dirtyNodes).GroupBy(
    (node) => node.document.Uri
  )
...
connection.onRequest(ConfigChangedRequestType, ({ configDatum, data }) => {
  ...
  for (const [version, props] of Object.entries(data)) {
    typeInfoMaps.set(
      version,
      new TypeInfoMap(objToTypeInfos(props.rawTypeInfo))
    )
  }
...
function onProjectChange(event: ProjectEvent) {
  const result = ValidateDirtyNodes(
    [...event.dirtyNodes.values()],
    event.project
  )
...
function GetProject(uri: DocumentUri): Project | undefined {
  for (const proj of projects.values()) {
    const xmlDoc = proj.DefDocuments.GetDefDocument(uri)
    if (xmlDoc) return proj
  }
}

function GetDefDocs(uri: DocumentUri): DefDocument[] {
  return [...projects.values()]
    .map((p) => p.DefDocuments.GetDefDocument(uri))
...
connection.onReferences(({ position, textDocument: { uri } }) => {
  ...
  if (textDoc) {
    for (const defDoc of defDocs) {
      ...
      function AddLocations(nodes: Node[]): void {
        for (const node of nodes) {
          ...
          if (textDoc)
            result.push({
              range: {
                start: textDoc.positionAt(node.start),
                end: textDoc.positionAt(node.end),
              },
      ...
      if (isWeakRefNode(node)) AddLocations([...node.weakReference.in.values()])
      if (isReferencedDef(node)) AddLocations([...node.derived.values()])
    }
  }
