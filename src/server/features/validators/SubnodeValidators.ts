...
export function checkInappropriateNode(
  this: NodeValidatorContext,
  node: typeNode
): ValidationResult {
  ...
  if (childNodes && childNodes.size > 0) {
    result.diagnostics = []
    const wrongChilds = node.children
      .filter((c) => c.closed && c.tag && !childNodes.has(c.tag.content))
      .map<Diagnostic>((n) => ({
...
export function checkDuplicateNode(
  this: NodeValidatorContext,
  node: typeNode
): ValidationResult {
...
export function checkInvalidNode(
  this: NodeValidatorContext,
  node: typeNode
): ValidationResult {
  const result: ValidationResult = { diagnostics: [] }
