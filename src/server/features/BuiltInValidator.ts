import {
  NodeValidationParticipant,
  NodeValidateFunction,
...
function getOrCreate(key: string): NodeValidateFunction[] {
  if (!builtInValidatorMap.has(key)) builtInValidatorMap.set(key, [])
...
for (const data of TypeToFunction) {
  for (const type of data.types) {
    ...
    const funcs = data.functions.filter(
      (func) => !arr.find((d) => d.name === func.name)
    ) // 랙 만들 가능성 있음... 개선하자
    arr.push(...funcs)
  }
}
...
function checkParentDefValid(
  this: NodeValidatorContext,
  node: typeNode
): ValidationResult {
...
function checkInvalidDefNode(
  ...
  if (typeInfo.specialType?.defType) {
    ...
    if (this.defDatabase && defName)
      if (
        node.parent?.tag?.content !== "Defs" &&
        isWeakRefNode(node) &&
        node.weakReference.out.size == 0
      )
        // root Def node is not a target
        result.diagnostics = [
          {
