...
export function doComplete({
  ...
  // TODO - fix this
  const collectDefNodeValueSuggestions = (
        ...
        if (typeInfo.specialType.defType) {
          ...
          if (defDatabase) {
            ...
            return {
              isIncomplete: false,
              items: defs.map((def) => ({
                label: def.children.find(
                  (node) => node.tag?.content === "defName"
                )?.text?.content!,
              })),
            }
          }
  ...
  function collectOpenTagSuggestions(
    ...
    if (isTypeNode(parentNode)) {
      ...
      if (
        ...
        !typeInfo.specialType.enumerable.isSpecial
      ) {
        result.items.push({
          label: "li",
        })
        ...
        if (typeInfo.childNodes) {
          const nodes = [...typeInfo.childNodes.keys()].map<CompletionItem>(
            (name) => ({ label: name, kind: CompletionItemKind.Field })
          )
          result.items = nodes
        }
  ...
  function collectDefTagSuggestions(): CompletionList {
    ...
    result.items = typeInfoMap.getDefNames().map(
      (name) =>
        ({
          label: name,
      ...
      case "Class":
        {
          if (
            ...
            parent &&
            isTypeNode(parent) &&
            parent.typeInfo.specialType?.enumerable
          ) {
            const genericType =
              parent.typeInfo.specialType.enumerable.genericType
            ...
            if (typeInfo) {
              ...
              if (name) {
                ...
                result.items = AsEnumerable(suggestions)
                  .Select(([name, _]) => name)
                  .Select(
                    (name) =>
                      ({
    ...
    switch (token) {
      case TokenType.StartTagOpen:
        if (scanner.getTokenEnd() === offset) {
          // <
          if (node.parent?.tag?.content === "Defs") {
            // XXXDef
      ...
      case TokenType.EndTagOpen:
        if (
          lastToken === TokenType.StartTagClose &&
