...
export function parseColor(input: string): color | parseColorErrorCode {
  ...
  if (stream.peekChar(0) !== _OPT) return parseColorErrorCode.notStartWithOPT
  ...
  while (!stream.eos()) {
    if (stream.advanceIfChar(_OPT)) {
      ...
      else return parseColorErrorCode.invalidOPT
    } else if (stream.advanceIfChar(_CPT)) {
      if (openParenthesis && stream.peekChar(1) === _EOS) {
        // end with )
        openParenthesis = false
        // complete
      } else {
        // unexpected ) middle of text
      ...
      if (stream.advanceUntilRegExp(/,|\)/)) {
        // matches , or )  |=position -> 123,| , 123)|
        const integerStr = input.substr(offset, stream.pos() - offset) // without the last one
        if (isInteger(integerStr)) {
          // must be a byte
          if (colors.length < 4) colors.push(integerStr)
          else return parseColorErrorCode.tooManyNumberText
        } else {
  ...
  if (stream.peekChar(-1) !== _CPT) return parseColorErrorCode.notEndWithCPT
