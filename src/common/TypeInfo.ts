...
export class TypeInfoMap extends Map<string, TypeInfo> {
  ...
  *getComps(
  ...
  ): Generator<[string, TypeInfo], void, unknown> {
    if (baseType) {
...
export class TypeInfoInjector {
  ...
  private _convertInternal(node: typeNode): void {
    ...
    while (queue.length > 0) {
      ...
      if (
        typeInfo.specialType?.enumerable &&
        typeInfo.specialType.enumerable.isSpecial !== true
      ) {
      ...
      for (const childNode of node.children) {
        ...
        const childTypeInfo = this.typeInfoMap.getByTypeIdentifier(
          childTypeInfoIdentifier
        )
...
export function getDefName(def: def): string | null {
  const defName = def.children.find((node) => node.tag?.content === "defName")
    ?.text?.content
  return defName || null
}
