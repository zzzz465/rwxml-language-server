 ...
 */
export function extractTypeInfos(
  ...
  const clientPath = process.env.isWebpack
    ? context.asAbsolutePath("./dist/client")
    : context.asAbsolutePath("./out/client")
  ...
  return new Promise((resolve, err) => {
    ...
    switch (platform()) {
      case "win32":
        extractorProcess = spawn(extractorPath, [
          ...args,
          "--OutputMode",
          "stdoutBytes",
          "--log",
    ...
    if (extractorProcess) {
      ...
      extractorProcess.stderr.on("data", (buffer: Buffer) => {
        ...
        vscode.window.showErrorMessage(
          `encounted an error while parsing data, ${errmsg}`
        )
        err(errmsg)
      })
      ...
      extractorProcess.on("exit", (code) => {
        if (code === 0) {
          ...
          try {
            // is stdout still alive after the process exit???
          ...
          vscode.window
            .showErrorMessage(
              "Failed to extract typeInfos from assemblies",
              "show log"
            )
            .then((text) => {
              if (text === "show log") {
      ...
      extractorProcess.on("error", (errmsg) => {
        // catch stderr
        vscode.window.showErrorMessage(
          `cannot create extractor process, err: ${errmsg}`
        )
        err(errmsg)
      })
    } else {
      vscode.window.showErrorMessage(
        `current platform ${platform()} is not supported.`
      )
    }
  })
}
